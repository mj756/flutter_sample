...
Json_serialization:
To generate model class run following command

    flutter packages pub run build_runner build


to read sha1 key from jks file run following command from gradle directory of your project
...
* For generate AppBundle => flutter build appbundle

* Generate APK from AppBundle:
    Path => hubl_flutter_app\build\app\outputs\bundle\release
    Command => java -jar "D:\bundletool.jar" build-apks --bundle=app-release.aab --output=hubl_app.apks --ks="D:\E drive backup\2. App Data\Beee app\beee_app_keystore.jks" --ks-pass=pass:android --ks-key-alias=beee_app --key-pass=pass:android
    Install APK on device =>  java -jar "D:\bundletool.jar" install-apks --apks=hubl_app.apks
